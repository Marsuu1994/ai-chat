...
**Entry point:** User clicks "Edit Plan" button on the board header → navigates to `/kanban/plans/[id]`.

**Template removal with instance cleanup:**
When user removes a task template from the plan and submits, show a confirmation modal:
- "Remove tasks from board?" — lists removed template names
- Two choices:
  - **Remove from board** — deletes task instances matching:
    - `templateId` in the removed set
    - `status IN (TODO, DOING)` — only incomplete board-visible tasks
    - EXPIRED tasks are ignored (past daily history)
    - DONE tasks are always preserved (completed effort is never erased)
...
- New weekly templates: generate all instances immediately (same as plan creation)

**Points accuracy after update:**
Option C remains accurate under both removal choices because:
- EXPIRED instances are not affected by either choice
- DONE instances are preserved either way
- Future daily projection updates immediately from new template snapshot
- Weekly instances are regenerated by `updatePlanAction` on template change
...
2. **Week Points** — `{weekDonePoints} / {weekProjectedPoints}`. Projected total = past daily instances + future daily projection from current templates + all weekly instances. Color: info/blue.
...
**Week Progress Bar (right, fills remaining space):** Horizontal bar with gradient fill (blue → green) showing `weekDoneCount / weekProjectedCount` as a percentage. Label: "Week Progress · {done} of {total} tasks".
- `weekDoneCount`: all tasks with `status = DONE` in the plan
- `weekProjectedCount`: past daily task count + projected future daily count (from current templates) + all weekly instances
...
BoardData {
  ...
  weekProjectedCount: number           // Projected total: past daily instances + future daily projection + all weekly instances
  weekDonePoints:     number           // Points from DONE tasks
  weekProjectedPoints:number           // Projected total points (same formula as count)
...
remainingDays      = daysBetween(today, weekEnd) + 1   // includes today through Sunday

currentDailyTemplates = plan.planTemplates.filter(pt => pt.template.type == DAILY)
dailyFuturePoints  = currentDailyTemplates.sum(t => t.template.points * t.template.frequency) * remainingDays
dailyFutureCount   = currentDailyTemplates.sum(t => t.template.frequency) * remainingDays
...
**Key design properties of Option C:**
- Today is counted in the future portion only — never double counted
- `weekDonePoints` and `weekDoneCount` are append-only — never decrease due to plan changes
- Past effort is preserved regardless of template edits
- Future projection always reflects the current plan snapshot
