...
### 2026-02-24
- Designed full ad-hoc task flow: updated `baseline.md` (schema: `planId` nullable, ad-hoc entity definition), `flows.md` (ad-hoc creation flow, updated create/edit plan flows with ad-hoc task selection/unlinking), and `api.md` (new `createAdhocTaskAction`, ad-hoc handling in create/edit plan actions, new DAL functions, ad-hoc in projected points)
- Merged `mockup-template-modal.html` and `mockup-adhoc-task-modal.html` into unified `mockup-task-modal.html` with 3 tabs: New Template, Edit Template, Add Ad-hoc Task — ad-hoc screen shows dimmed board background, lightning bolt info banner, and "Add to Board" button
- Updated all board/plan-form/review-changes mockups to include ad-hoc task elements (board mockup: ad-hoc task cards, ad-hoc "+" button; plan form: ad-hoc tasks section; review changes: ad-hoc added/removed sections)
